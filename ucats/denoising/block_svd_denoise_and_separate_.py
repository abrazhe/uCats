...
def block_svd_denoise_and_separate(data,
                                   stride=2,
                                   nhood=5,
                                   ncomp=None,
                                   min_comps=1,
    ...
    def _process_loc(r, c, mask):
        # don't need center here
        sl = (slice(r - nhood, r + nhood), slice(c - nhood, c + nhood))
        ...
        if ncomp is None:
            ...
            rank = max(min_comps, rank)
            rank = min(max_comps, rank)
            #rank = np.int(min(max(min_comps, max_comps, min_ncomp(s, patch.shape) + 1)))
        ...
        if spatial_filter >= 1:
            ...
            Wx_b = np.array([
                smoothed_medianf(f, spatial_filter / 5, spatial_filter) for f in W_images
            ])
        ...
        if only_truncated_svd:
            ...
            if baseline_smoothness:
                baseline_kw = dict(plow=50, out_smooth=baseline_smoothness)
                biases = np.array([
                    percentile_baseline(v, ns=mad_std(v), **baseline_kw) for v in svd_signals
                ])
                #biases = np.array([smoothed_medianf(v, smooth=5, wmedian=int(baseline_smoothness)) for v in svd_signals])
            else:
                biases = [find_bias(v, ns=mad_std(v)) for v in svd_signals]
            ...
            active_comps = np.sum(np.abs(signals_filtered) > 0, 1) > 3
            # %active component for clustering is such that was active at least for k frames
            ...
            if not np.any(active_comps):
                ...
                if not with_clusters:
                    if correct_spatial_components:
                        ...
                        Xdiff_permuted = np.array(
                            [np.random.permutation(v) for v in Xdiff.T]).T
                        signals_permuted = np.array(
                            [np.random.permutation(v) for v in signals_filtered])
                        ...
                        Wmasks = np.array([
                            threshold_object_size(
                                np.abs(f1) > 3 * np.percentile(np.abs(f2), 99), 5)
                    ...
                    cx = skclust.AgglomerativeClustering(nclusters,
                                                         affinity='l1',
                                                         linkage='average')
                    ...
                    cluster_signals = np.array(
                        [approx_c.T[affs == k].mean(0) for k in np.unique(affs)])
                    #cbiases = np.array([find_bias(v) for v in cluster_signals])
                    labeler = partial(percentile_label,
                                      percentile_low=cluster_detection_plow,
                                      tau=2)
                    csignals_filtered = np.array([
                        simple_pipeline_(v, noise_sigma=mad_std(v), labeler=labeler)
                        for v in cluster_signals
                    ])
                    som_spatial_comps = np.array([affs == k for k in np.unique(affs)])
                    rec = csignals_filtered.T @ som_spatial_comps
                    rec = rec.reshape(-1, *psh)
...
def block_svd_separate_tslices(frames,
                               twindow=200,
                               nhood=5,
                               ncomp=None,
                               mask_of_interest=None,
    ...
    if twindow < L:
        tslices = [slice(i, i + twindow) for i in range(0, L - twindow, twindow // 2)]
        tslices = tslices + [slice(L - twindow, L)]
    else:
        tslices = [slice(0, L)]
    counts = np.zeros(L)
    ...
    for k, ts, m in zip(range(L), tslices, mask_list):
        s, b = block_svd_denoise_and_separate(frames[ts],
                                              mask_of_interest=m,
                                              nhood=nhood,
                                              ncomp=ncomp,
                                              **denoiser_kw)
