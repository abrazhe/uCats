...
from ..utils import smoothed_medianf, mad_std, find_bias
    ...
    def _process_loc(r, c, mask):
        ...
        if only_truncated_svd:
            ...
            if baseline_smoothness:
                biases = np.array(
                    # TODO: switch for multi_scale_simple_baseline???
                    [simple_baseline(v, plow=50, smooth=baseline_smoothness, ns=mad_std(v)) for v in svd_signals])
            ...
            active_comps = np.sum(np.abs(signals_filtered) > 0,
                1) > 3  # %active component for clustering is such that was active at least for k frames
