...
from .baselines import multi_scale_simple_baseline, calculate_baseline_pca_asym
from .denoising import patch_pca_denoise2
...
def svd_denoise_tslices(dfof, twindow, mask_of_interest, temporal_filter, verbose):
    pass
...
def make_enh5(dfof, twindow=50, nhood=5, stride=2, temporal_filter=3, verbose=False):
    ...
    dfof_denoised = svd_denoise_tslices(dfof, twindow, mask_of_interest=amask, temporal_filter=temporal_filter,
        verbose=verbose)
...
def make_denoising_animation(frames, yhat, f0, movie_name, start_loc=None, path=None):
    ...
    if (start_loc is None):
        f = ndi.gaussian_filter(np.max(yhat - f0, 0), 3)
    ...
    def _animate(frame_ind):
        ...
        loc[0] = np.nasarray((loc[0][0] % nrows, loc[0][1] % ncols), np.int)
...
def roticity_fft(data, period_low=100, period_high=5, npc=6):
    ...
    s2 = s ** 2 / (s ** 2).sum()
    u = (u - u.mean(0))[:, :npc]
    p = (abs(np.fft.fft(u, axis=0)) ** 2)[:L // 2]
    nu = np.fft.fftfreq(len(data))[:L // 2]
