...
def make_enh4(frames,
    ...
    if kind.lower() == 'corr':
        coll = signals_from_array_correlation(frames,
                                              stride=stride,
                                              nhood=nhood,
...
def make_denoising_animation(frames, yhat, f0, movie_name, start_loc=None, path=None):
    ...
    if path is None:
        locs = (loc + np.cumsum([
            (0, 0)
        ] + [np.random.randint(-1, 2, size=2) * 0.5 + (1.25, 1.25) for i in range(L)],
                                axis=0)).astype(int)
    else:
        locs = []
    ...
    ly = axbottom.plot(yhat[xsl], color='royalblue',
                       label=r'$\hat F$')[0]    #,animated=True)
    lb = axbottom.plot(f0[xsl], color=(0.2, 0.8, 0.5), lw=2,
                       label='F0')[0]    #,animated=True)
